...
%% QP
if feedFunction(t) == 0
    w = p(11);   %Fraction of optimum growth that cell follows - BATCH[-]
else
    w = p(12);  %Fraction of optimum growth that cell follows - FEDBATCH [-]
end
...
QPproblem.F             = 2*w*eye(N);                  %Objective quadratic term
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         if t==0
%             save('metMovie_FB_3_fix.mat','fluxDistrib')
%         else
%             if length(fluxDistrib)<1000
%                 fluxDistrib = [zeros(length(model.rxns),1);fluxDistrib];
%             end
%             S = load('metMovie_FB_3_fix.mat','fluxDistrib');
