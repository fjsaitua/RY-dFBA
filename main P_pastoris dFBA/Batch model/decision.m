...
ksensit  = zeros(1,m);
kidentif = zeros(1,m);
for i = 1:m % for all the parameters
    %Sensitivity:
    sensitive = false;
    for j = 1:n % for all the state variables
        if Ms2(i,j) >= 0.01; % Will be true if a parameter is sensitive for at least 1 state variable
            sensitive = true;
        end
    end
    if ~sensitive
