...
function model = kineticConstraints_PATIL(model,t,x,excRxn,k)
...
if E < 1e-3
    vEtOH = 0;
    if ateEtOH == 0 && t > t_feed && superUmbral == 0
        assignin('base','V0_FB',x(1))
        assignin('base','X0_FB',x(2))
        assignin('base','t_feed_carb',t)
        assignin('base','ateEtOH',1)
        display(['Time EtOH: ' num2str(t)])
    end
end

if P < 1e-3 
    vPyr = 0;
    if atePyr == 0 && t > t_feed && superUmbral == 0
        assignin('base','V0_FB',x(1))
        assignin('base','X0_FB',x(2))
        assignin('base','t_feed_carb',t)
        assignin('base','atePyr',1)
        display(['Time Pyr: ' num2str(t)])
    end
end

if A < 1e-3
    vArab = 0;
    if ateArab == 0 && t > t_feed && superUmbral == 0
        assignin('base','V0_FB',x(1))
        assignin('base','X0_FB',x(2))
        assignin('base','t_feed_carb',t)
        assignin('base','ateArab',1)
        display(['Time Arab: ' num2str(t)])
    end
end
...
if feedFunction_PATIL(t) == 0
    ...
    model = changeRxnBounds(model,model.rxns(excRxn(8,1)),v*vTAU,'l');
end
